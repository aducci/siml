...
export function validate(model: SimlRoot, mode: 'strict' | 'loose' = 'loose'): ValidationError[] {
...
function validateAction(action: string, target: Component, addError: (msg: string, code: string) => void) {
    // Mapping of allowed actions per type
    const allowedActions: Record<ComponentType, string[]> = {

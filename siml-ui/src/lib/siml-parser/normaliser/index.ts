import { SimlRoot, Component, SequenceStep, ComponentType } from '../types';

export function normalize(model: SimlRoot): SimlRoot {
    // Deep clone to avoid mutating original
    const normalized = JSON.parse(JSON.stringify(model)) as SimlRoot;
...
function normalizeStep(step: SequenceStep, components: Record<string, Component>): SequenceStep {
    const newStep = { ...step };
            ...
            if (targetType === 'database') {
                ...
                if (!newStep.operation && newStep.action) {
                    ...
                    if (['read', 'write'].includes(newStep.action)) {
                        newStep.operation = newStep.action as any;
                    }
                }
